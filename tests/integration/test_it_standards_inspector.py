...
from standards_inspector.repositories import GitLabRepository
...
def gitlab_checker():
    ...
    return StandardsInspector(GitLabRepository(url, token, test_project_id))
...
def test_check_standards(gitlab_checker, test_project_id):
    ...
    if not test_project_id:
        pytest.skip(
            "GITLAB_TEST_PROJECT_ID or GITLAB_PROJECT_ID environment variable not set"
        )
